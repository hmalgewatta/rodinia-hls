...
#pragma HLS INTERFACE m_axi port=SEQA offset=slave bundle=gmem max_read_burst_length=64 depth=128
#pragma HLS INTERFACE m_axi port=SEQB offset=slave bundle=gmem max_read_burst_length=64 depth=128
#pragma HLS INTERFACE m_axi port=alignedA offset=slave bundle=gmem max_read_burst_length=64 depth=128
#pragma HLS INTERFACE m_axi port=alignedB offset=slave bundle=gmem max_read_burst_length=64 depth=128
...
//#pragma HLS INTERFACE m_axi port=SEQA offset=slave bundle=gmem0 depth=641
//#pragma HLS INTERFACE m_axi port=SEQB offset=slave bundle=gmem1 depth=641
//#pragma HLS INTERFACE m_axi port=alignedA offset=slave bundle=gmem2 depth=641
//#pragma HLS INTERFACE m_axi port=alignedB offset=slave bundle=gmem3 depth=641
//#pragma HLS INTERFACE s_axilite port=SEQA bundle=control
//#pragma HLS INTERFACE s_axilite port=SEQB bundle=control
//#pragma HLS INTERFACE s_axilite port=alignedA bundle=control
//#pragma HLS INTERFACE s_axilite port=alignedB bundle=control
...
void needwun(char SEQA[ALEN], char SEQB[BLEN],
    ...
    trace: while(a_idx>0 || b_idx>0) {
        r = b_idx*(ALEN+1);
        if (ptr[r + a_idx] == ALIGN && b_idx > 0 && a_idx > 0){
            alignedA[a_str_idx++] = SEQA[a_idx-1];
            alignedB[b_str_idx++] = SEQB[b_idx-1];
            a_idx--;
            b_idx--;
        }
        else if (ptr[r + a_idx] == SKIPB && a_idx > 0){
            alignedA[a_str_idx++] = SEQA[a_idx-1];
            alignedB[b_str_idx++] = '-';
            a_idx--;
        }
        else if (b_idx > 0){ // SKIPA
            alignedA[a_str_idx++] = '-';
            alignedB[b_str_idx++] = SEQB[b_idx-1];
            b_idx--;
        }
...
extern "C" {
//void workload(char* SEQA, char* SEQB,
//             char* alignedA, char* alignedB, int num_jobs, char* alignedA_ref, int* res) {
void workload(char SEQA[ALEN*NUM_JOBS], char SEQB[BLEN*NUM_JOBS],
             char alignedA[(ALEN+BLEN)*NUM_JOBS], char alignedB[(ALEN+BLEN)*NUM_JOBS], int num_jobs) {
